...
def evaluate_model(
    ...
    overall_acc = np.sum(per_class_correct) / np.sum(per_class_total)
    ...
    print(f"[INFO] Overall accuracy: {overall_acc:.4f}")

    return model, overall_acc


# ------------------------------------
# Main script for evaluating all models
# ------------------------------------
def main():
    ...
    # Specify model paths from the image
    model_paths = [
        # r"D:\FYP\MODELS\VGGModel\Improved_20250210\best_model_vgg_improved_20250210.pth",
        # r"D:\FYP\MODELS\VGGModel\Improved_20250210\current_model_vgg_improved.pth",
        # r"D:\FYP\MODELS\VGGModel\Improved_20250210\final_model_vgg_improved_20250210.pth",
]

    # Set output directory for confusion matrices
    output_dir = r"D:\FYP\ConfusionMatrixOutputs\VGG_Improved_20250210"
    os.makedirs(output_dir, exist_ok=True)
    ...
    for mp in model_paths:
        ...
        model, overall_acc = evaluate_model(
            model_path=mp,
            data_dir=data_dir,
    ...
    if results:
        ...
        print("\n" + "=" * 50)
        print(f"[BEST MODEL] {best_model['model_name']} with accuracy {best_model['accuracy']:.4f}")
        print("=" * 50)
