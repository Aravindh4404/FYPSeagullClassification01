...
SCATTER_OUTPUT = OUTPUT_DIR / "upperwing_vs_wingtip_scatter.png"
BAR_OUTPUT = OUTPUT_DIR / "upperwing_vs_wingtip_bar.png"
DARK_BOX_OUTPUT = OUTPUT_DIR / "dark_pixel_percentage_box.png"
WING_DIST_OUTPUT = OUTPUT_DIR / "wing_intensity_distribution.png"
WINGTIP_DIST_OUTPUT = OUTPUT_DIR / "wingtip_intensity_distribution.png"
...
METRIC_COLORS = {
    "Upperparts mean": "#4C78A8",
    "Primaries mean": "#E15759",
}
...
def load_intensity_data() -> pd.DataFrame:
    ...
    # Assuming your CSV has columns like: mean_wing_intensity_method1, mean_wingtip_intensity_method1, etc.
    # We'll need to identify which columns to use

    # Check for available columns and select appropriate ones
    wing_cols = [col for col in df.columns if 'mean_wing' in col.lower()]
...
def ensure_dark_pixel_percentage(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    preferred_column = "pct_wingtip_darker_than_wing"
    if preferred_column in df.columns:
        df["pct_darker_pixels"] = df[preferred_column]
        return df
...
def print_dataset_summary(df: pd.DataFrame) -> None:
    ...
    total_rows = len(df)
    print("\nDATA SUMMARY: all.csv (filtered wingtip)")
    ...
    for species, subset in grouped:
        ...
        print(
            f"  {label}: n={count}, "
            f"upperwing mean={wing_mean:.2f}, wingtip mean={wingtip_mean:.2f}"
        )

    overall_wing = df["mean_wing_intensity"].mean()
    overall_wingtip = df["mean_wingtip_intensity"].mean()
    print(
        f"Overall means → upperwing: {overall_wing:.2f}, "
        f"wingtip: {overall_wingtip:.2f}\n"
    )
...
def plot_upperwing_vs_wingtip(df: pd.DataFrame) -> None:
    ...
    ax.plot(
        ...
        color="#2f2f2f",
        linewidth=1.4,
        label="Upperwing = Wingtip",
    )

    ax.set_xlim(0, 255)
    ax.set_ylim(0, 255)
    ax.set_aspect("equal", "box")
    ax.set_xlabel("Upperwing mean intensity (0–255)")
    ax.set_ylabel("Wingtip mean intensity (0–255)")
    ax.set_title(
        "Relationship Between Upperwing and Wingtip Tones (per bird)",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_species_bar_chart(df: pd.DataFrame) -> None:
    ...
    ax.set_ylim(0, 270)
    ax.set_title(
        "Average Upperwing vs Wingtip Intensity (filtered dark-pixel data)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_dark_pixel_boxplot(df: pd.DataFrame) -> None:
    """Create a box plot of dark pixel percentages per species."""
    ...
    palette = [SPECIES_COLORS[s] for s in box_df["species"].unique() if s in SPECIES_COLORS]
    sns.boxplot(
        data=box_df,
        x="species_display",
        y=column,
    ...
    ax.set_ylabel("Wingtip dark pixel percentage (%)")
    ax.set_xlabel("Species")
    ax.set_ylim(0, min(100, box_df[column].max() + 5))

    fig.tight_layout()
    fig.savefig(DARK_BOX_OUTPUT, dpi=300)
...
def plot_intensity_distribution(df: pd.DataFrame, column: str, title: str, output_path: Path) -> None:
    ...
    fig.text(
        0.5, 0.02,
        f'Bin Width: {BIN_WIDTH} units | Intensity reference shows actual brightness values',
        ha='center',
        va='bottom',
        fontsize=9,
        style='italic'
...
def main() -> None:
    ...
    plot_intensity_distribution(
        df,
        'mean_wing_intensity',
        'Wing Intensity Distribution',
        WING_DIST_OUTPUT
    )
    plot_intensity_distribution(
        df,
        'mean_wingtip_intensity',
        'Wingtip Intensity Distribution',
        WINGTIP_DIST_OUTPUT
    )
