...
DATA_PATH = BASE_DIR / "D:\FYPSeagullClassification01\Features_Analysis\Intensity\Data_Analysis\Wingtip_Dark_Pixel_Visualizations\dark_pixel_results_all_images.csv"  # Changed to use all.csv
...
def print_dataset_summary(df: pd.DataFrame) -> None:
    ...
    total_rows = len(df)
    print("\nDATA SUMMARY: all.csv (filtered primaries)")
...
def plot_upperwing_vs_wingtip(df: pd.DataFrame) -> None:
    ...
    ax.set_title(
        "Relationship Between Upperparts and Primaries Tones (per bird)",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_species_bar_chart(df: pd.DataFrame) -> None:
    ...
    ax.set_ylim(0, 270)
    ax.set_title(
        "Average Upperparts vs Primaries Intensity (filtered dark-pixel data)",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_dark_pixel_boxplot(df: pd.DataFrame) -> None:
    ...
    ax.set_title('Primaries Dark Pixel Analysis by Species', fontsize=12, fontweight='bold', pad=12)
    ax.tick_params(axis='x', rotation=45)
    ax.grid(True, alpha=0.3)

    ymin = 0  # or max(0, box_df[column].min() - 5) if you want a tiny margin
