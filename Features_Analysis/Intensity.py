...
# 1. CONFIGURATION
###############################################################################
# Adjust these paths to match where your images and segmentation maps are stored.
...
def load_images_and_seg_maps(sb_img_dir, sb_seg_dir, gw_img_dir, gw_seg_dir, s):
    ...
    """
    # Limit file reads to first s images
    ...
    for img_name, seg_name in zip(sb_images, sb_segs):
        ...
        # Convert BGRA → BGR if there's an alpha channel
...
def extract_region_pixels(image, seg_map, region_color, tolerance=10):
    ...
    # BGR color boundaries with tolerance
    ...
    # Create mask from the color range
